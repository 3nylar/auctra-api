...
declare module "fastify" {
  interface FastifyRequest {
    auth?: { orgId: string; keyId: string; scopes: string[] };
  }
}
...
export async function authenticate(req: FastifyRequest, _reply: FastifyReply): Promise<void> {
  ...
  if (raw.startsWith("sk_live_") || raw.startsWith("sk_test_")) {
    if (!raw.startsWith(KEY_PREFIX)) throw errors.wrongEnvironment(raw.slice(0, 8));
  } else {
    throw errors.invalidKey();
  }

  const key = await prisma.apiKey.findUnique({ where: { hash: hashKey(raw) } });
  if (!key || key.revokedAt) throw errors.invalidKey();
...
export function requireScope(scope: string) {
  return async (req: FastifyRequest) => {
    ...
    if (!req.auth.scopes.includes(scope)) throw errors.missingScope(scope);
  };
}
