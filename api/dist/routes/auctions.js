...
export async function auctionRoutes(app) {
    // ---- List -------------------------------------------------------------
    ...
    app.get("/v1/auctions/:id", { preHandler: requireScope("auctions:read") }, async (req) => {
        ...
        if (!auction)
            throw errors.notFound("auction", id);
    ...
    app.post("/v1/auctions", { preHandler: requireScope("auctions:write") }, async (req, reply) => {
        ...
        if (body.mode === "managed" && !managedSignerEnabled)
            throw errors.managedSignerDisabled();
        return withIdempotency(req, reply, req.auth.orgId, async () => {
            const tx = await prepare({
                functionName: "createAuction",
                args: [body.token_contract, BigInt(body.token_id), reserve, BigInt(body.duration_seconds)],
                from: body.seller,
            });
            const now = new Date();
    ...
    app.post("/v1/auctions/:id/bids", { preHandler: requireScope("auctions:write") }, async (req, reply) => {
        ...
        if (status !== "live" && status !== "ending")
            throw errors.auctionNotLive(status);
        ...
        if (amount < min)
            throw errors.bidBelowMinimum(min.toString());
        return withIdempotency(req, reply, req.auth.orgId, async () => {
            ...
            return {
                ...
                body: {
                    ...
                    next_step: "Sign transaction_request with the bidder's wallet. A bid.placed webhook fires once it confirms.",
                },
            };
        });
    });
    ...
    app.post("/v1/auctions/:id/settle", { preHandler: requireScope("auctions:write") }, async (req, reply) => {
        ...
        if (new Date() < auction.endTime)
            throw errors.auctionNotEnded(auction.endTime.toISOString());
        return withIdempotency(req, reply, req.auth.orgId, async () => {
    ...
    app.post("/v1/auctions/:id/cancel", { preHandler: requireScope("auctions:write") }, async (req, reply) => {
        ...
        if (!auction)
            throw errors.notFound("auction", id);
        if (BigInt(auction.highestBidWei) > 0n)
            throw errors.auctionHasBids();
        return withIdempotency(req, reply, req.auth.orgId, async () => {
